...
def predict_two_classes(examples: List[FactExample], fact_checker):
    ...
    # liste pentru analiza din Partea 3
    false_positives = []  # model: S, gold: NS
    false_negatives = []  # model: NS, gold: S
    ...
    for i, example in enumerate(tqdm(examples)):
        ...
        # colectăm FP / FN pentru analiza de erori
        if gold_label == gold_label_indexer.index("NS") and pred_label == gold_label_indexer.index("S"):
            # false positive: gold = NS, pred = S
            false_positives.append((example, converted_label, raw_pred))
        elif gold_label == gold_label_indexer.index("S") and pred_label == gold_label_indexer.index("NS"):
            # false negative: gold = S, pred = NS
            false_negatives.append((example, converted_label, raw_pred))

    # afișăm statisticile standard (accuracy, F1 etc.)
    print_eval_stats(confusion_mat, gold_label_indexer)

    # afișăm câteva exemple pentru Partea 3
    print("\n==== FALSE POSITIVES (gold=NS, pred=S) ====")
    for ex, gold, pred in false_positives[:10]:
        print("FACT:", ex.fact)
        print("GOLD:", gold, "PRED:", pred)
        print("PASSAGE TITLES:", [p["title"] for p in ex.passages])
        print("----")

    print("\n==== FALSE NEGATIVES (gold=S, pred=NS) ====")
    for ex, gold, pred in false_negatives[:10]:
        print("FACT:", ex.fact)
        print("GOLD:", gold, "PRED:", pred)
        print("PASSAGE TITLES:", [p["title"] for p in ex.passages])
        print("----")

    # dacă vrei să știi și câte sunt în total:
